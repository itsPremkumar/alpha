...
## Self-documentation retrieval contract

`packages/harness/alpha/knowledge/self_documentation.py` owns the free,
stdlib-only `search_project_docs` capability. It indexes only allowlisted
project guidance, product docs, shipped config/schema files, and public skill
docs; `references/` is opt-in and carries `design_reference` authority. Runtime
config, secrets, private skills, dependency trees, symlinks, and generated
output are excluded. The tool's hidden runtime and server-side root resolver
must prevent model-selected project roots, and the model-visible schema must not
expose forced refresh. Search returns bounded snippets, line ranges, and a
content SHA-256; `read` is allowlist-confined and fails with `source_changed`
when the supplied digest no longer matches. The implementation must not add a
network, embedding, paid API, or durable user-data dependency without an
explicit product decision and tests. Regression coverage lives in
`backend/tests/test_self_documentation_index.py`.

## Autonomy truth & recovery contract

`packages/harness/alpha/ops/autonomy_truth.py` owns the free, stdlib-only
`autonomy_control` capability. It must fail closed: readiness evidence comes
only from the injected runtime and server configuration, never model-authored
claims; missing required observations are `unknown`; optional capability limits
do not block the core offline loop; and external side effects remain
server-approval-gated. Failure classification must redact secrets, bound output,
and distinguish safe retry from escalation. Activity digests may summarize
bounded counts and hashed path references, but must never echo raw event
payloads, file paths, credentials, or tool output. The `recovery` action returns
only a redacted, bounded current-thread resume brief; it must not expose raw
continuity records or imply that missing history is complete. The tool must not call
providers, networks, or persistence layers. `build_autonomous_plan` must embed
this same server-owned preflight and gate board/profile mutations when the core
is not ready. Regression coverage lives in `backend/tests/test_autonomy_truth.py`
and `backend/tests/test_autonomous_planner.py`.

## Reversible mutation contract

`packages/harness/alpha/safety/reversible_delete.py` owns the free,
stdlib-only `reversible_delete` capability. Planning is read-only with respect
to user files; execution accepts only a server-resolved, human-approved
project request hash-bound to the exact plan and a bounded batch; the tool's
hidden runtime binds operations to the authenticated per-user, per-thread
workspace; project IDs are storage-safe; symlinks/traversal are rejected,
fingerprints are rechecked before mutation, and files move to a local quarantine
