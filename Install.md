...
This file is for coding agents. If the Agent Workspace repository is not already cloned and open, clone `https://github.com/bytedance/agent-workspace.git` first, then continue from the repository root.

## Goal

Bootstrap a Agent Workspace local development workspace on the user's machine with the least risky path available.
...
- The Agent Workspace repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch Agent Workspace.
...
- If the current directory is not the Agent Workspace repository root, clone `https://github.com/bytedance/agent-workspace.git` if needed, then change into the repository root.
- Confirm the current directory is the Agent Workspace repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start Agent Workspace
