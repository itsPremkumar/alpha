...
from agent_workspace_extension_api import EXTENSION_PRINCIPAL_RESOLVER_KEY, ExtensionPrincipal
...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
    ...
    # Security headers last: last-registered user middleware is outermost, so
    # these headers also cover auth/CSRF rejections and the trace fallback
    # 500 generated by inner layers. See security_headers_middleware.py.
    ...
    # (or when the explicit AGENT_WORKSPACE_ALLOW_UNVERIFIED_GITHUB_WEBHOOKS=1
    ...
    if github_webhooks.is_route_enabled():
        ...
        logger.warning("GitHub webhooks route NOT mounted: GITHUB_WEBHOOK_SECRET unset and AGENT_WORKSPACE_ALLOW_UNVERIFIED_GITHUB_WEBHOOKS not set. /api/webhooks/github will respond 404. Configure either env var to enable the route.")
