...
This guide explains how to configure Alpha for your environment.
...
- On macOS, Alpha does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
`knowledge_search`. Alpha does not persist a copy of dataset or document
...
it contains RAGFlow dataset IDs selected by the deployment operator, Alpha
...
scores from different embedding spaces are not globally comparable, Alpha
...
deletion remain in RAGFlow and are not exposed as Agent tools or Alpha APIs.
...
one name, and on duplicate names Alpha keeps the **first** configured
...
here shipped in v1.4.9. Alpha does not persist any index
...
structured entities, relationships, chunks, and references; Alpha keeps the
...
mutation remain in LightRAG and are not exposed as Agent tools or Alpha
APIs.
...
Alpha supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:
...
When using Docker development (`make docker-start`), Alpha starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.

Remote/provisioner backends default to explicit file synchronization because
Alpha cannot infer whether their `/mnt/user-data` mount points reference the
...
If the configured `host_path` is not visible to the gateway process, Alpha logs an error and ignores that mount.
...
When you configure `sandbox.mounts`, Alpha exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.
...
out of the internal bridge. Alpha also sets the upstream AIO image's
...
**Deny**, **Allow temporarily**, and **Allow for this sandbox** choices. Alpha
...
engines. Alpha applies Engine 28's isolated bridge gateway mode to both IPv4
...
Alpha versions are recognized when they use the configured image and retain
...
published. Alpha generates a separate relay token for each sandbox, requires
...
`AioSandboxProvider` talks to the sandbox container through the `agent-sandbox` SDK. The Dockerfile for the default `enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest` image is not part of this repository; Alpha treats that image as an upstream AIO sandbox runtime.
...
USER root
# Example user dependency; not required by Alpha itself.
RUN apt-get update \
...
If you rebuild the runtime from scratch instead of extending the published image, it must expose the same HTTP API used by `agent-sandbox`. Alpha currently depends on:

- `sandbox.get_context()`, including `home_dir`
- `shell.exec_command(...)`
- `bash.exec(...)` — only exercised for per-command environment injection (skills that declare `required-secrets`). The `/v1/bash/*` routes exist since upstream all-in-one-sandbox `1.9.3`; on older images (including a `latest` tag still frozen on the `1.0.0.x` line) Alpha fails fast with an actionable error instead of surfacing the raw 404. Pin `sandbox.image` to `1.9.3` or newer (e.g. `1.11.0`) and recreate the sandbox container to use `required-secrets` with the AIO sandbox.
- `file.read_file(...)`
...
- `/mnt/user-data` must remain writable because Alpha mounts thread workspace, uploads, and outputs there.
- `home_dir` comes from the sandbox context endpoint; do not assume Alpha hardcodes it.
- Shell command handling must remain compatible with serialized `exec_command` calls. Alpha serializes shell access on the host side to avoid corrupting the sandbox's persistent shell session.
...
`.`/`..`, or a path that contains or sits below Alpha's reserved mounts
...
2. Restart the Alpha service to apply changes

## Environment Variables

Alpha supports environment variable substitution using the `$` prefix:
...
Alpha executes agent-generated shell/code through a configurable sandbox
...
matters for Alpha because the gateway executes model-generated commands, so a
...
Alpha can reuse your Claude Code / Codex CLI subscription login as a model
...
adapters authenticate independently of Alpha via their own documented env —
...
Docker container. Alpha forwards enabled MCP servers to the MCode session.
