...
`alpha.runtime.runs.verification` provides deterministic checks for optional
run acceptance criteria and independently collected evidence. It is an overlay
only: `RunManager` remains the sole lifecycle owner. Gateway run requests may
persist validated `acceptance_criteria` into run metadata, but a terminal
`success` must not be presented as verified unless the verifier has enough
passing evidence of every required kind. Tests: `tests/test_run_acceptance_criteria.py`.
...
`alpha.benchmarks.release_gate` applies fail-closed, provider-neutral numeric
promotion gates to versioned benchmark measurements. Its defaults cover task
success, authorization isolation, recovery, and prompt-injection resistance;
CI/deployment must supply real benchmark values rather than treating defaults
as evidence. Tests: `tests/test_release_gate.py`.
...
`alpha.knowledge.self_documentation` provides the stdlib-only, offline
`search_project_docs` tool. It indexes current repository guidance, docs, and
shipped config/schema files with authority labels, returns line-addressable
snippets plus SHA-256 evidence, and supports bounded digest-checked reads. Its
hidden runtime selects the server project root; do not expose `project_root` or
forced refresh in the model schema.
Runtime config, secrets, private skills, dependency trees, and symlinks are
excluded. `references/` is opt-in and always returned as non-authoritative
`design_reference` material. Do not introduce network, embedding, paid API, or
persistent user-data dependencies into this path without an explicit product
decision and regression tests. Tests:
`tests/test_self_documentation_index.py`.

## Autonomy truth & recovery

`alpha.ops.autonomy_truth` and the `autonomy_control` built-in provide offline,
fail-closed readiness decisions, explicit capability boundaries, redacted
failure recovery plans, and bounded activity digests. The tool's hidden runtime
and server config are the only readiness evidence; model-authored context and
approval claims are ignored. Optional sandbox/vision/network/MCP/skill limits
do not block the required offline model/tool/documentation loop. External side
effects require server-resolved approval; retryability is bounded; raw
event/error payloads, file paths, and secrets are never returned. The recovery
action exposes only a redacted, bounded resume brief and distinguishes missing
continuity from verified history. Do not add
provider, network, or persistence dependencies to this decision layer.
`build_autonomous_plan` embeds the same preflight and blocks board/profile
mutations until the core is ready. Tests: `tests/test_autonomy_truth.py`,
`tests/test_autonomous_planner.py`.

## Reversible mutation safety

`alpha.safety.reversible_delete` and the `reversible_delete` built-in provide
read-only planning, server-resolved project approval, bounded batches, a hidden
runtime bound to the authenticated per-user/per-thread workspace, storage-safe
project IDs, root-self rejection, symlink/traversal rejection, fingerprint checks, receipts, rollback,
and restoration. The service must never hard-delete, call a network trash
provider, or bypass existing policy/approval middleware. Tests:
`tests/test_reversible_delete.py`.
