...
Do not report `fixed` from a repair-kind label or a diagnostic suggestion.
Future repair adapters must perform a scoped action and collect independent
post-action runtime evidence before success is permitted. Existing refusal
reasons and record serialization remain supported. Regression coverage lives
in `tests/test_workforce_intelligence.py`.
