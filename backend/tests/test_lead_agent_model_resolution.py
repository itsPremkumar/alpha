...
def test_make_lead_agent_reads_runtime_options_from_context(monkeypatch):
    ...
    get_available_tools.assert_called_once_with(model_name="context-model", groups=None, include_mcp=True, subagent_enabled=True, app_config=app_config)
    assert result["model"] is not None
...
def test_create_summarization_middleware_uses_configured_model_alias(monkeypatch):
    ...
    monkeypatch.setattr(summarization_middleware_module, "AgentWorkspaceSummarizationMiddleware", lambda **kwargs: kwargs)
...
def test_create_summarization_middleware_uses_frontend_supported_update_key(monkeypatch):
    ...
    assert update_key == "AgentWorkspaceSummarizationMiddleware.before_model"
