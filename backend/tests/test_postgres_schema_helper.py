...
from alpha.persistence.postgres_schema import (
...
class TestBuildAsyncpgConnectArgs:
    def test_sets_search_path_for_schema(self):
        assert build_asyncpg_connect_args("alpha") == {"server_settings": {"search_path": "alpha"}}
...
class TestBuildPsycopgOptions:
    def test_builds_libpq_options(self):
        assert build_psycopg_options("alpha") == "-c search_path=alpha"
...
class TestCreateSchemaSql:
    def test_builds_create_schema_statement(self):
        assert create_schema_sql("alpha") == 'CREATE SCHEMA IF NOT EXISTS "alpha"'

    def test_empty_schema_returns_none(self):
        assert create_schema_sql("") is None

    @pytest.mark.parametrize("schema", ['a"; DROP SCHEMA public; --', "MySchema", "a b", "alpha\n"])
...
class TestDsnWithSearchPath:
    ...
    def test_appends_options_query_encoded(self):
        dsn = "postgresql://u:p@h:5432/db"
        out = dsn_with_search_path(dsn, "alpha")
    ...
    def test_preserves_existing_options_query(self):
        ...
        out = dsn_with_search_path(dsn, "alpha")
        query = parse_qs(urlsplit(out).query)
        assert query["options"] == ["-c statement_timeout=5000 -c search_path=alpha"]
    ...
    def test_supports_keyword_dsn(self):
        ...
        dsn = "host=localhost dbname=alpha user=postgres"
        out = dsn_with_search_path(dsn, "alpha")
        assert conninfo_to_dict(out) == {
            "host": "localhost",
            "dbname": "alpha",
            "user": "postgres",
            "options": "-c search_path=alpha",
        }
    ...
    def test_preserves_keyword_dsn_options(self):
        ...
        dsn = "host=localhost dbname=alpha options='-c statement_timeout=5000'"
        out = dsn_with_search_path(dsn, "alpha")
        assert conninfo_to_dict(out)["options"] == "-c statement_timeout=5000 -c search_path=alpha"
    ...
    def test_rejects_non_postgres_url_scheme(self):
        try:
            dsn_with_search_path("mysql://localhost/db", "alpha")
    ...
    def test_preserves_option_value_containing_space(self):
        ...
        from alpha.persistence.postgres_schema import _merge_search_path_option

        merged = _merge_search_path_option(r"-c application_name=My\ App", "alpha")
        assert "'" not in merged
        assert r"application_name=My\ App" in merged
        assert merged.endswith("-c search_path=alpha")
    ...
    def test_preserves_option_value_containing_tab(self):
        ...
        merged = _merge_search_path_option("-c application_name=My\\\tApp", "alpha")
        assert "'" not in merged
...
class TestNormalizeLibpqDsn:
    ...
    def test_leaves_keyword_dsn_unchanged(self):
        dsn = "host=localhost dbname=alpha"
        assert normalize_libpq_dsn(dsn) == dsn
