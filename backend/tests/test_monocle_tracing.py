...
from agent_workspace.config import is_monocle_tracing_enabled
from agent_workspace.config.tracing_config import get_tracing_config, reset_tracing_config
...
def clear_monocle_env(monkeypatch):
    ...
    monkeypatch.setattr("agent_workspace.tracing.monocle._setup_completed", False)
...
def test_embedded_hint_when_enabled_but_uninitialized(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="agent_workspace.tracing.factory"):
...
def test_no_import_time_setup():
    ...
    script = textwrap.dedent(
        ...
        import agent_workspace.agents as agents
