...
importing ``alpha.agents`` no longer sets up telemetry at import time.
"""
...
from alpha.config import is_monocle_tracing_enabled
from alpha.config.tracing_config import get_tracing_config, reset_tracing_config
...
def clear_monocle_env(monkeypatch):
    ...
    monkeypatch.setattr("alpha.tracing.monocle._setup_completed", False)
...
def test_embedded_hint_when_enabled_but_uninitialized(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="alpha.tracing.factory"):
...
def test_no_import_time_setup():
    """Regression: importing alpha.agents must not install telemetry.

    The setup call used to live at module import in ``alpha/agents/__init__``.
