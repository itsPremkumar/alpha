...
def test_install_skill_archive_runs_security_scan(monkeypatch, tmp_path):
    ...
    config = SimpleNamespace(
        skills=SimpleNamespace(get_skills_path=lambda: skills_root, container_path="/mnt/skills", use="agent_workspace.skills.storage.local_skill_storage:LocalSkillStorage"),
...
def test_custom_skill_delete_continues_when_history_write_is_readonly(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agent_workspace.skills.storage.user_scoped_skill_storage.UserScopedSkillStorage.append_history", _readonly_history)
    monkeypatch.setattr("app.gateway.routers.skills.refresh_user_skills_system_prompt_cache_async", _refresh)
    monkeypatch.setattr("app.gateway.routers.skills.get_effective_user_id", lambda: "default")
...
def test_custom_skill_delete_fails_when_skill_dir_removal_fails(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agent_workspace.skills.storage.local_skill_storage.shutil.rmtree", _fail_rmtree)
    monkeypatch.setattr("app.gateway.routers.skills.refresh_user_skills_system_prompt_cache_async", _refresh)
    monkeypatch.setattr("app.gateway.routers.skills.get_effective_user_id", lambda: "default")
...
def test_update_skill_refreshes_prompt_cache_before_return(monkeypatch, tmp_path):
    ...
    from agent_workspace.skills.storage import user_scoped_skill_storage as uss_module
    ...
    monkeypatch.setattr("agent_workspace.skills.storage.user_scoped_skill_storage.UserScopedSkillStorage", _FakeUserScopedStorage)
...
def test_public_skill_toggle_rebuilds_projection_before_response(monkeypatch, tmp_path):
    from agent_workspace.config.extensions_config import ExtensionsConfig, SkillStateConfig, reset_extensions_config, set_extensions_config
    from agent_workspace.config.paths import Paths
    from agent_workspace.skills.projection import rebuild_skill_projections
