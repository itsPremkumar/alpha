...
def test_apply_prompt_template_threads_explicit_app_config_without_global_config(monkeypatch):
    ...
    explicit_config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=mounts),
        skills=SimpleNamespace(container_path="/mnt/explicit-skills", use="alpha.skills.storage.local_skill_storage:LocalSkillStorage", get_skills_path=lambda: Path("/tmp/explicit-skills")),
        skill_evolution=SimpleNamespace(enabled=False),
        tool_search=SimpleNamespace(enabled=False),
    ...
    monkeypatch.setattr("alpha.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr("alpha.config.memory_config.get_memory_config", fail_get_memory_config)
    monkeypatch.setattr(prompt_module, "get_or_new_skill_storage", lambda app_config=None: SimpleNamespace(load_skills=lambda enabled_only=True: []))
    monkeypatch.setattr(prompt_module, "get_or_new_user_skill_storage", lambda user_id, app_config=None: SimpleNamespace(load_skills=lambda *, enabled_only: []))
    monkeypatch.setattr(prompt_module, "get_agent_soul", lambda agent_name=None, **kwargs: "")
...
def test_apply_prompt_template_threads_explicit_app_config_to_subagents_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("alpha.config.get_app_config", fail_get_app_config)
    monkeypatch.setattr("alpha.config.subagents_config.get_subagents_app_config", fail_get_subagents_app_config)
    monkeypatch.setattr(prompt_module, "get_or_new_skill_storage", lambda app_config=None: SimpleNamespace(load_skills=lambda enabled_only=True: []))
    monkeypatch.setattr(prompt_module, "get_agent_soul", lambda agent_name=None, **kwargs: "")
...
def test_get_memory_context_uses_explicit_app_config_without_global_config(monkeypatch):
    ...
    monkeypatch.setattr("alpha.config.memory_config.get_memory_config", fail_get_memory_config)
    monkeypatch.setattr("alpha.runtime.user_context.resolve_runtime_user_id", lambda runtime: "user-1")
...
def test_get_memory_context_propagates_required_read_error_without_backend_config(monkeypatch):
    ...
    monkeypatch.setattr("alpha.agents.memory.get_memory_manager", lambda: manager)
    monkeypatch.setattr("alpha.runtime.user_context.get_effective_user_id", lambda: "user-1")
...
def test_get_memory_context_swallows_ordinary_manager_error(monkeypatch):
    from alpha.agents.memory import MemoryManagerError
...
def test_get_memory_context_prefers_explicit_user_id(monkeypatch):
    ...
    monkeypatch.setattr("alpha.runtime.user_context.resolve_runtime_user_id", fail_resolve_runtime_user_id)
    monkeypatch.setattr(
