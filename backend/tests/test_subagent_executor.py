...
class TestSubagentTracingWiring:
    ...
    def _make_executor(self, classes, *, user_id=None, name="general-purpose", parent_model="test-model", agent_workspace_trace_id=None):
        SubagentExecutor = classes["SubagentExecutor"]
        SubagentConfig = classes["SubagentConfig"]
        config = SubagentConfig(
            name=name,
    ...
    def test_deerflow_trace_id_falls_back_to_the_ambient_trace(self, classes):
        ...
        assert executor.agent_workspace_trace_id == "ambient-trace-1"
    ...
    async def test_aexecute_injects_langfuse_session_user_and_trace_name(
        ...
        executor = self._make_executor(classes, user_id="alice", name="general_purpose", agent_workspace_trace_id="gateway-trace-sub")
        fake_agent = _FakeStreamAgent()
        monkeypatch.setattr(executor, "_build_initial_state", self._noop_build_initial_state)
        monkeypatch.setattr(executor, "_create_agent", lambda *a, **kw: fake_agent)
        ...
        assert metadata.get("agent_workspace_trace_id") == "gateway-trace-sub"
        assert fake_agent.captured_context.get("agent_workspace_trace_id") == "gateway-trace-sub"
    ...
    async def test_environment_tag_emitted_from_deer_flow_env(
        ...
        """``AGENT_WORKSPACE_ENV`` must surface as an ``env:<value>`` tag so Langfuse
...
class TestSubagentGuardrailAttribution:
    ...
    async def test_aexecute_propagates_channel_user_id_to_subagent_context(
        ...
        dispatching turn's ``AGENT_WORKSPACE_CHANNEL_USER_ID`` (group chats share one
