# Alpha behavioural tests (Monocle Test Tools)

Trace-based tests for Alpha. Monocle records each run as a structured trace
...
- **One offline example** (`test_assertion_api_example`) loads a recorded trace
  ...
  format and the asserter wiring, not Alpha's behaviour. Treat it as the
...
it embeds the Alpha system prompt as of the recording date and the content
...
they still skip if the Alpha app is not importable or `config.yaml` is
...
so there is no hard-coded key requirement. Alpha's `web_search` is
...
1. Run Alpha under Monocle and capture a trace of a run you are happy with
