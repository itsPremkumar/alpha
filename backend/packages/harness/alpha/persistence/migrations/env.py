...
try:
    import alpha.persistence.models as models  # register ORM models with Base.metadata

    _ = models
except ImportError:
    # Models not available — migration will work with existing metadata only.
    logging.getLogger(__name__).warning("Could not import alpha.persistence.models; Alembic may not detect all tables")
...
async def run_migrations_online() -> None:
    ...
    # ``alpha.persistence.engine`` sets this on its own connections, but
