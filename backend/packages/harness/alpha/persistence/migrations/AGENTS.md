...
| legacy (Alpha tables, no `alembic_version`) | `create_all` (baseline tables only, backfill) + `alembic stamp 0001_baseline` + `upgrade head` |
...
- `migrations/versions/0025_run_recovery_index.py` - adds the composite `(status, stop_reason)` lookup index used by the bounded safe auto-resume scanner. It adds no columns or data backfill and is idempotent for empty/bootstrap-created databases.
