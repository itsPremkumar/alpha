...
Lives at the top-level package (not ``alpha.runtime``) so it can be
imported from ``alpha.agents.thread_state`` without pulling in the heavy
``alpha.runtime`` package __init__ (which eagerly imports the runs
machinery). Anchored from ``alpha.agents.thread_state`` so every process
that builds a Alpha graph (gateway, workers, in-process LangGraph
...
def _as_overwrite(value: Any) -> tuple[bool, Any]:
    ...
    Alpha's write paths produce for these Union channels (the branch and
    ...
    an internal serialization detail Alpha never emits into these channels.
    """
...
def _binop_first_write_stores_overwrite_wrapper() -> bool:
    ...
    channel.key = "alpha-overwrite-probe"
