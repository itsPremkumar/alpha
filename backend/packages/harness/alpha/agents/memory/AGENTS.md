...
Operators must stop Alpha and snapshot the storage root before migration.
...
One API key is bound to one configured Alpha owner.
Reject another owner before remote access.

Alpha owns capture timing, the recall query, and the transcript cursor.
The package owns transport, message conversion, batching, and Session commits.
One Alpha thread maps to one stable OpenViking Session.
