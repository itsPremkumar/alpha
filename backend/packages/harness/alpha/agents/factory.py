...
from alpha.agents.thread_state import adapt_state_schema_for_mode, get_thread_state_schema, normalize_middleware_state_schemas
from alpha.config.database_config import CheckpointChannelMode
from alpha.tools.builtins import ask_clarification_tool
    ...
    *,
    ...
    if feat.memory is not False:
        if isinstance(feat.memory, AgentMiddleware):
            ...
            from alpha.config.memory_config import get_memory_config, should_use_memory_tools
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from alpha.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        if isinstance(feat.subagent, AgentMiddleware):
            ...
            from alpha.config.subagents_config import DEFAULT_MAX_TOTAL_SUBAGENTS_PER_RUN
        ...
        from alpha.tools.builtins import task_tool

        if subagent_runtime is None:
            extra_tools.append(task_tool)
        else:
            from alpha.tools.builtins.task_tool import bind_task_tool
        ...
        if subagent_runtime is not None and subagent_runtime.batch_submitter is not None:
            from alpha.tools.builtins.batch_task_tool import bind_batch_tools
            ...
            if is_subagent_batch_runtime_available():
                from alpha.tools.builtins import batch_status, batch_task, cancel_batch
