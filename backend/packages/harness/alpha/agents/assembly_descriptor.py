...
def _tool_source(tool: object) -> str:
    ...
    if module.startswith("alpha.tools.builtins") or module.startswith("alpha.agents.memory"):
        return "builtin"
    if "skill" in module:
        return "skill"
...
def _unwrap_middleware(middleware: object) -> tuple[object, str | None]:
    ...
    ``alpha.extensions`` sits below this layer, so importing it here would
...
def build_assembly_descriptor(
    *,
    ...
    prompt_template_id: str = "alpha-lead-agent-v1",
    tools: list[object],
