...
def _subagent_release_policy(
    ...
    *,
    ...
    from alpha.subagents import get_available_subagent_names, get_subagent_config
...
def _append_memory_tools_without_name_conflicts(tools: list) -> None:
    ...
    from alpha.agents.memory.tools import get_memory_tools
...
def build_middlewares(
    ...
    *,
    ...
    if should_use_memory_tools(resolved_app_config.memory):
        from alpha.agents.memory.manager import backend_requires_passive_writes_in_tool_mode
    ...
    from alpha.config.learning_fork_config import get_learning_fork_config
    ...
    if deferred_setup is not None and deferred_setup.deferred_names:
        ...
        from alpha.agents.middlewares.mcp_routing_middleware import assert_mcp_routing_before_deferred_filter
...
def _load_enabled_available_skills(available_skills: set[str] | None, *, app_config: AppConfig, user_id: str | None = None) -> list[Skill]:
    try:
        from alpha.agents.lead_agent.prompt import get_enabled_skills_for_config
...
def _complete_assembly(
    *,
    ...
    """
    from alpha.diagnostics.invariants import verify_agent_assembly
    from alpha.extensions import get_agent_build_extensions
...
def _assemble_lead_agent(config: RunnableConfig, *, app_config: AppConfig) -> LeadAgentAssembly:
    ...
    from alpha.tools.builtins import setup_agent, update_agent
    ...
    from alpha.config.agent_preset_config import preset_summary, resolve_agent_preset
