...
import alpha.checkpoint_patches as _checkpoint_patches  # noqa: F401 - import-time saver fixes
...
def _resolve_snapshot_frequency(snapshot_frequency: int | None) -> int:
    ...
    else default. Imported lazily — ``alpha.runtime.__init__`` reaches this
    module via ``checkpoint_state``, so a top-level import would cycle."""
    if snapshot_frequency is not None:
        return snapshot_frequency
    from alpha.runtime.checkpoint_mode import resolve_checkpoint_snapshot_frequency
