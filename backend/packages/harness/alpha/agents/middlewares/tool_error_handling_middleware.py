...
def _build_runtime_middlewares(
    *,
    ...
    if getattr(app_config, "review_guard", None) and app_config.review_guard.enabled:
        from alpha.agents.middlewares.review_guard_middleware import build_review_guard_middleware
    ...
    if getattr(app_config, "hooks", None) is not None and app_config.hooks.enabled:
        from alpha.agents.middlewares.hooks_bridge_middleware import HooksBridgeMiddleware

        tail.append(HooksBridgeMiddleware(config=app_config.hooks))

    middlewares = [*outer_wrappers, *thread_hooks, *tail]

    # Ordering invariants are declared in alpha.extensions.ordering and
...
def build_subagent_runtime_middlewares(
    *,
    ...
    from alpha.agents.middlewares.summarization_middleware import create_summarization_middleware
