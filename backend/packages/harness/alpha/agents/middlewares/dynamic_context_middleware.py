...
from alpha.utils.messages import INJECTED_USER_MESSAGE_ID_SUFFIX, strip_injected_user_message_id_suffix
...
# are defined in ``alpha.utils.messages`` and re-exported here, where the
...
class DynamicContextMiddleware(AgentMiddleware):
    ...
    def _build_full_reminder(self, runtime: Runtime | None = None) -> tuple[str, str | None]:
        ...
        try:
            from alpha.bots.dm import build_bot_roster_reminder, resolve_runtime_bot_name
        ...
        try:
            from alpha.context_files import load_context_files, resolve_runtime_repo_root
    ...
    def _read_failures_are_fatal(self, *, allow_io: bool = True) -> bool | None:
        from alpha.agents.memory import memory_read_failures_are_fatal
