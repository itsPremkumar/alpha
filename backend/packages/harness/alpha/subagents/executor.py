...
from alpha.agents.middlewares.audit_context import LOOP_DETECTION_RECORDER_CONTEXT_KEY, TOOL_PROMOTION_RECORDER_CONTEXT_KEY
from alpha.agents.thread_state import SandboxState, ThreadDataState, ThreadState
from alpha.authz.principal import normalize_authz_attributes
...
from alpha.subagents.config import (
    DEFAULT_SUBAGENT_TOOL_ALLOWLIST,
    SubagentConfig,
    resolve_subagent_model_name,
)
from alpha.subagents.context_snapshot import SNAPSHOT_SYSTEM_NOTE, ParentContextSnapshot
...
if TYPE_CHECKING:
    ...
    # `from alpha.subagents import SubagentExecutor`, which re-enters this
...
# Kept as wire keys here instead of importing ``alpha.sandbox`` at module
...
def _utcnow() -> datetime:
    ...
    # lifecycle metadata never depends on the host wall clock (see alpha.utils.time).
...
def _harvest_tool_receipts(
    ...
    *,
    ...
    ``alpha.agents``; resolving ``tool_receipt`` at call time keeps module
    ...
    if not messages:
        return None
    try:
        from alpha.agents.middlewares.tool_receipt import extract_citing_turn_receipts, extract_tool_receipts
...
def _harvest_shell_persistence(final_state: Any) -> bool | None:
    ...
    """
    try:
        from alpha.sandbox.overwrite import unwrap_sandbox
...
class SubagentExecutor:
    ...
    def _create_agent(
        ...
        *,
        ...
        if deferred_setup is not None and deferred_setup.deferred_names:
            from alpha.tools.builtins.tool_search import build_mcp_routing_middleware
        ...
        *,
        ...
        try:
            ...
            descriptor = build_assembly_descriptor(
                ...
                prompt_template_id="alpha-subagent-v1",
                tools=tools,
                middlewares=middlewares,
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        from alpha.tools.builtins.tool_search import assemble_deferred_tools, get_deferred_tools_prompt_section, get_mcp_routing_hints_prompt_section
        ...
        from alpha.skills.describe import build_skill_search_setup, get_skill_index_prompt_section
    ...
    async def _aexecute_admitted(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            if execution_context is not None and execution_context.get("sandbox_id") is not None:
                try:
                    from alpha.sandbox import get_sandbox_provider
