...
from alpha.subagents.report_contract import MAX_ACCEPTANCE_CRITERIA, normalize_acceptance_criteria
...
def _resolve_scoped_path(path: str, thread_data: Mapping[str, Any] | None, *, resolve_symlinks: bool = False) -> str | None:
    ...
    # imported in cycles with alpha.tools (same pattern as report_contract).
    from alpha.sandbox.tools import replace_virtual_path
...
def _probe_file_size(runtime: Any, resolved: str, thread_data: Mapping[str, Any] | None) -> int | None:
    ...
    """
    try:
        from alpha.sandbox.tools import _resolve_local_read_path, ensure_sandbox_initialized, is_local_sandbox
...
def _check_file_leaf(
    ...
    *,
    ...
    # package is imported in cycles with alpha.tools (same pattern as
    # report_contract).
    from alpha.sandbox.exceptions import SandboxError, SandboxFileNotFoundError
