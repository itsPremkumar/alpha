...
Runs Alpha sandboxes on [OpenSandbox](https://github.com/opensandbox-group/OpenSandbox),
using the synchronous Python SDK behind Alpha's `Sandbox` and
...
`OPEN_SANDBOX_DOMAIN` are set. `use_server_proxy` is useful when Alpha can
...
Create returns only after the SDK readiness check and Alpha's
...
The full Alpha surface is implemented:
...
- `execute_command` forwards per-call environment variables and positive
  ...
  non-zero exit information in Alpha's string result.
