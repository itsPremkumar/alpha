"""``BoxliteBox`` — Alpha :class:`Sandbox` backed by a BoxLite micro-VM.
...
``asyncio.to_thread`` worker Alpha invokes us from.

Every operation is a shell command run inside the box (``cat`` / ``find`` /
``grep`` / chunked ``base64``), parsed with the shared ``alpha.sandbox.search``
...
class BoxliteBox(Sandbox):
    ...
    Args:
        id: Alpha-side sandbox id (the BoxLite box id).
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        # so Alpha's virtual paths are used as-is; we only reject traversal.
    ...
    def execute_command(
        ...
        Alpha passes a bash command *string*; BoxLite's ``exec`` takes argv, so
