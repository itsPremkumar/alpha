...
from alpha.agents.goal_state import GoalEvaluation, GoalState
from alpha.agents.middlewares.input_sanitization_middleware import neutralize_untrusted_tags
from alpha.config.app_config import AppConfig
from alpha.config.database_config import CheckpointChannelMode
from alpha.constants import TOOL_RESULTS_DIRNAME
...
from alpha.runtime.events.message_identity import attach_message_seq, message_identity
...
class _SubagentEventBuffer:
    ...
    async def add(self, chunk: Any) -> None:
        ...
        if self._event_store is None:
            return
        # Lazy import: importing alpha.subagents at module load triggers its
        # package __init__ (executor → agents → tools → task_tool), which imports
        # back from alpha.subagents and deadlocks at gateway startup. Deferring
        # it to call time (after all modules are loaded) breaks that cycle.
        from alpha.subagents.step_events import subagent_run_event
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    from alpha.sandbox.lease import release_sandbox_execution_lease_async
