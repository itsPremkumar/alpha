...
from alpha.config.agents_config import load_agent_config, preserve_non_managed_fields, validate_agent_name
from alpha.config.app_config import get_app_config
from alpha.config.paths import get_paths
from alpha.persistence.agents import get_agent_store
...
# ``alpha.agents.lead_agent.agent``); this set is the in-tool mirror
...
def update_agent(
    ...
    # ``alpha.agents.lead_agent.agent``). The same channel set is
