...
**System One provider boundary** (`system_one`):

- `provider` accepts only `vercel-gateway`, `typesafe`, or `laya`; an unknown value fails config validation instead of silently falling back to the hosted gateway.
- Provider defaults are applied only when the corresponding field is omitted. Hosted defaults are the Vercel `/v1/evaluate` and `typesafe-ai/jev`; direct TypeSafe is `https://api.typesafe.ai/v1` + `jev-latest`; Laya is `http://127.0.0.1:8000` + an empty model for its language-aware Router. Explicit `base_url`/`model` values always win.
- Laya is self-hosted and keyless on loopback by default. If `LAYA_API_KEY` is configured on the server, set the same value through `system_one.api_key` (`$LAYA_API_KEY`); never reuse a cloud gateway key for the local provider.
- Laya preflight caps are deliberate: the client abstains on state/question/choice cardinality beyond `laya_max_state_chars`, `laya_max_request_chars`, `laya_max_questions`, and `laya_max_choice_options` because Laya's context and option token budgets are smaller than Jev's. This preserves the existing heuristic/LLM fallback instead of scoring truncated evidence. `laya_max_partition_requests` and `laya_max_partition_latency_ms` bound the local partition/final tournament; exceeding either returns `None`, and `evaluate_choice_partitioned()` retains every original option for ranking without presenting partition scores as global probabilities.
- The default `SystemOneClient` reads the hot-reloadable `AppConfig.system_one` on each access. Its calibration JSONL includes `provider`; old records remain readable with an empty/unknown provider bucket.
...
- `voice` - Local real-time speech contract: `routing.mode` defaults to `local_only`; STT/TTS engine settings, safe Piper voice ID/operator model paths, and bounded streaming frame/endpoint/session limits. Cached local engines key off these values so hot reload does not require rebuilding every request; assets themselves come from `make voice-setup`.
...
- `hooks` - Shell-hook bridge (`enabled`, `hooks_path`, `default_timeout_seconds`, `fail_open`); reuses Claude/Codex `hooks.json` command hooks at SessionStart/UserPromptSubmit/PreToolUse/PostToolUse/SessionEnd. Disabled by default; operator-trusted file
