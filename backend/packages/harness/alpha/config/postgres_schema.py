...
# A ``$``-anchored ``re.match`` would accept a trailing newline (``"alpha\n"``):
# Python's ``$`` matches just before a single trailing ``\n``, which then
# creates a *quoted* schema literally named ``alpha\n`` while the *unquoted*
# ``search_path`` folds to ``alpha`` and misses it, silently landing tables
# in ``public``.
