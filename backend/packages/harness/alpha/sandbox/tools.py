...
from alpha.agents.thread_state import ThreadDataState
from alpha.authz.sandbox_authz import (
...
from alpha.sandbox.file_operation_lock import get_file_operation_lock
...
from alpha.sandbox.path_patterns import build_output_mask_pattern, normalize_mask_tail, replace_output_path_matches
...
def _get_mcp_allowed_paths() -> list[str]:
    ...
    allowed_paths = []
    try:
        from alpha.config.extensions_config import get_extensions_config
...
def _compiled_mask_patterns(sources: tuple[tuple[str, str], ...]) -> tuple[tuple[re.Pattern[str], str, str], ...]:
    ...
    # ``alpha.sandbox.path_patterns`` so the static regex path and dynamic
...
def _lark_cli_env_from_runtime(runtime: Runtime, command: str, *, sandbox_paths: bool) -> dict[str, str] | None:
    ...
    if not _LARK_CLI_COMMAND_RE.search(command):
        return None
    try:
        from alpha.integrations.lark_cli import lark_cli_env_overlay, sandbox_lark_broker_active
...
def read_file_tool(
    ...
    try:
        ...
        try:
            ...
            max_chars = 50000
        if hashline:
            from alpha.sandbox.hashline import format_hashline, format_hashline_ranged
