...
deliberately kept as a sibling to :mod:`alpha.guardrails` rather than folded
...
2. **Run-time execution deny** — reuses :class:`~alpha.guardrails.middleware.GuardrailMiddleware`
   via a thin adapter (see :mod:`alpha.authz.adapter`), catching dynamic
