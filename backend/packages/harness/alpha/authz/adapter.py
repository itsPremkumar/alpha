...
This lets the existing :class:`~alpha.guardrails.middleware.GuardrailMiddleware`
enforce :class:`~alpha.authz.provider.AuthorizationProvider` decisions at
tool-call time — no new middleware class required (see RFC §6.1).

The adapter maps :class:`~alpha.guardrails.provider.GuardrailRequest`
fields to :class:`~alpha.authz.provider.AuthzRequest` fields, calls the
authorization provider, and converts the :class:`~alpha.authz.provider.AuthzDecision`
back to a :class:`~alpha.guardrails.provider.GuardrailDecision`.

Principal construction delegates to
:func:`~alpha.authz.principal.build_principal_from_context` so Layer 1
...
from alpha.authz.provider import AuthorizationProvider, AuthzDecision, AuthzRequest
...
class GuardrailAuthorizationAdapter:
    ...
    def evaluate(self, request: GuardrailRequest) -> GuardrailDecision:
        ...
        adapter is consumed by :class:`~alpha.guardrails.middleware.GuardrailMiddleware`,
