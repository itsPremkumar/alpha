...
from agent_workspace.agents.middlewares.audit_context import LOOP_DETECTION_RECORDER_CONTEXT_KEY, TOOL_PROMOTION_RECORDER_CONTEXT_KEY
from agent_workspace.agents.thread_state import SandboxState, ThreadDataState, ThreadState
from agent_workspace.authz.principal import normalize_authz_attributes
...
from agent_workspace.subagents.step_events import capture_new_step_messages
from agent_workspace.subagents.token_collector import SubagentTokenCollector
from agent_workspace.trace_context import AGENT_WORKSPACE_TRACE_METADATA_KEY, ensure_trace_context, resolve_trace_id
...
if TYPE_CHECKING:
    ...
    # `from agent_workspace.subagents import SubagentExecutor`, which re-enters this
...
def _harvest_tool_receipts(
    ...
    *,
    ...
    if not messages:
        return None
    try:
        from agent_workspace.agents.middlewares.tool_receipt import extract_citing_turn_receipts, extract_tool_receipts
...
def _harvest_shell_persistence(final_state: Any) -> bool | None:
    ...
    """
    try:
        from agent_workspace.sandbox.overwrite import unwrap_sandbox
...
class SubagentExecutor:
    ...
    def _create_agent(
        ...
        *,
        ...
        if deferred_setup is not None and deferred_setup.deferred_names:
            from agent_workspace.tools.builtins.tool_search import build_mcp_routing_middleware
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        from agent_workspace.tools.builtins.tool_search import assemble_deferred_tools, get_deferred_tools_prompt_section, get_mcp_routing_hints_prompt_section
        ...
        from agent_workspace.skills.describe import build_skill_search_setup, get_skill_index_prompt_section
    ...
    async def _aexecute_admitted(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        from agent_workspace.extensions import get_loaded_extensions
        ...
        try:
            ...
            if execution_context is not None and execution_context.get("sandbox_id") is not None:
                try:
                    from agent_workspace.sandbox import get_sandbox_provider
