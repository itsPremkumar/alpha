...
consumer side of this module lives in ``agent_workspace.persistence.*``):
...
def _storage_user_id_from_auth_identity(identity: object | None) -> str | None:
    ...
    # BaseUser.identity, while Agent Workspace's user directories require a narrower
...
def resolve_config_user_id(config: object | None) -> str:
    ...
    values. Gateway runtime context remains the next source for Agent Workspace's
