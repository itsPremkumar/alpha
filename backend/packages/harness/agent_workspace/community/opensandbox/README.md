...
Runs Agent Workspace sandboxes on [OpenSandbox](https://github.com/opensandbox-group/OpenSandbox),
using the synchronous Python SDK behind Agent Workspace's `Sandbox` and
...
`OPEN_SANDBOX_DOMAIN` are set. `use_server_proxy` is useful when Agent Workspace can
...
Create returns only after the SDK readiness check and Agent Workspace's
...
The full Agent Workspace surface is implemented:
...
- `execute_command` forwards per-call environment variables and positive
  ...
  non-zero exit information in Agent Workspace's string result.
...
- `list_dir`, `glob`, and `grep` use portable `find`/`grep` commands and the
  shared Agent Workspace result parsers.
