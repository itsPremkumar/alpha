...
from agent_workspace.models.fallback import FallbackChatModel
...
def _apply_stream_chunk_timeout_default(model_class: type, model_settings_from_config: dict) -> None:
    ...
    it is accepted by ``ChatOpenAI`` and by every Alpha provider that subclasses
...
def _build_single_model(
    ...
    # Drop Alpha metadata keys the provider constructor must never see
    ...
    from agent_workspace.models.openai_codex_provider import CodexChatModel
