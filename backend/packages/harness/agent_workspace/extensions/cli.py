...
from agent_workspace.extensions.manager import ExtensionManager
...
def find_project_root() -> Path:
    ...
    if configured:
        ...
        if (candidate / "backend" / "pyproject.toml").is_file():
            return candidate
        raise FileNotFoundError(f"AGENT_WORKSPACE_PROJECT_ROOT is not a DeerFlow checkout: {candidate}")
    ...
    for candidate in candidates:
        ...
        if (candidate / "backend" / "pyproject.toml").is_file():
            return candidate
    raise FileNotFoundError("could not find a DeerFlow checkout; set AGENT_WORKSPACE_PROJECT_ROOT")
