...
class DynamicToolSynthesizer:
    ...
    def _compile_ephemeral(
        ...
        module = types.ModuleType(f"agent_workspace.dynamic_tools.{name}")
...
class SysPathGuard:
    ...
    def __exit__(self, exc_type: Any, exc: Any, tb: Any) -> bool:
        ...
        if "agent_workspace.dynamic_tools" in sys.modules:
            del sys.modules["agent_workspace.dynamic_tools"]
        for key in [k for k in sys.modules if k.startswith("agent_workspace.dynamic_tools.")]:
            del sys.modules[key]
        return False
