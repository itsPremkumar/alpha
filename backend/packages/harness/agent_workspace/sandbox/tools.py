...
from agent_workspace.agents.thread_state import ThreadDataState
from agent_workspace.authz.sandbox_authz import (
...
from agent_workspace.sandbox.exceptions import (
    SandboxError,
    SandboxNotFoundError,
    SandboxRuntimeError,
)
from agent_workspace.sandbox.file_operation_lock import get_file_operation_lock
...
from agent_workspace.sandbox.path_patterns import build_output_mask_pattern, normalize_mask_tail, replace_output_path_matches
...
def _get_mcp_allowed_paths() -> list[str]:
    ...
    allowed_paths = []
    try:
        from agent_workspace.config.extensions_config import get_extensions_config
...
def _lark_cli_env_from_runtime(runtime: Runtime, command: str, *, sandbox_paths: bool) -> dict[str, str] | None:
    ...
    if not _LARK_CLI_COMMAND_RE.search(command):
        return None
    try:
        from agent_workspace.integrations.lark_cli import lark_cli_env_overlay, sandbox_lark_broker_active
...
def read_file_tool(
    ...
    try:
        ...
        try:
            ...
            max_chars = 50000
        if hashline:
            from agent_workspace.sandbox.hashline import format_hashline, format_hashline_ranged
