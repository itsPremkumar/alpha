...
from agent_workspace.config.learning_fork_config import LearningForkConfig, get_learning_fork_config
...
if TYPE_CHECKING:
    from agent_workspace.agents.memory import MemoryManager
    from agent_workspace.skills.proposals import SkillProposalStore
...
class LearningForkMiddleware(AgentMiddleware[LearningForkMiddlewareState]):
    ...
    async def after_agent(self, state: LearningForkMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        if self._config.defer_when_busy:
            ...
            from agent_workspace.learning.review_queue import get_review_queue
    ...
    async def _execute_fork(self, thread_id: str, user_id: str, trace_id: str, digest: str, lead_model_name: str | None) -> None:
        ...
        from agent_workspace.tools.builtins import add_memory, propose_skill_tool, recall_memory
            ...
            for tool_call in tool_calls:
                ...
                try:
                    if tool_name == "add_memory":
                        ...
                        from agent_workspace.skills.proposals import get_skill_proposal_store
