...
from agent_workspace.config.summarization_config import DEFAULT_SKILL_FILE_READ_TOOL_NAMES
...
def _build_runtime_middlewares(
    *,
    ...
    if getattr(app_config, "review_guard", None) and app_config.review_guard.enabled:
        from agent_workspace.agents.middlewares.review_guard_middleware import build_review_guard_middleware
    ...
    if getattr(app_config, "hooks", None) is not None and app_config.hooks.enabled:
        from agent_workspace.agents.middlewares.hooks_bridge_middleware import HooksBridgeMiddleware
...
def build_subagent_runtime_middlewares(
    *,
    ...
    from agent_workspace.agents.middlewares.summarization_middleware import create_summarization_middleware
