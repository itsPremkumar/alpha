...
from agent_workspace.constants import DEFAULT_SKILLS_CONTAINER_PATH
...
from agent_workspace.subagents import get_available_subagent_names
from agent_workspace.tools.builtins.tool_search import get_deferred_tools_prompt_section
...
def _build_available_subagents_description(available_names: list[str], bash_available: bool, *, app_config: AppConfig | None = None) -> str:
    ...
    from agent_workspace.subagents.registry import get_subagent_config
...
is user-managed data (visible and editable via the {display_name} UI) — you may
...
[citation:FastAPI](https://fastapi.tiangolo.com).
LangGraph supports workflow orchestration
[citation:LangGraph Docs](https://langchain.com/langgraph).
...
- [LangGraph Docs](https://langchain.com/langgraph) - Workflow orchestration documentation
...
- ✅ RIGHT in Sources: `[FastAPI](https://fastapi.tiangolo.com) - Official documentation`
...
def _get_memory_context(
    ...
    *,
    ...
    """
    from agent_workspace.agents.memory import MemoryManagerError, MemoryReadError

    config = None
    try:
        from agent_workspace.agents.memory import get_memory_manager
        from agent_workspace.runtime.user_context import resolve_runtime_user_id
...
def get_skills_prompt_section(
    ...
    *,
    ...
    if skill_names is not None:
        from agent_workspace.skills.describe import get_skill_index_prompt_section
...
def _build_acp_section(*, app_config: AppConfig | None = None) -> str:
    ...
    if app_config is None:
        try:
            from agent_workspace.config.acp_config import get_acp_agents
...
def apply_prompt_template(
    ...
    *,
    ...
    if subagent_enabled:
        from agent_workspace.subagents.batch_runtime import is_subagent_batch_runtime_available
