...
from agent_workspace.agents.task_continuity.tools import append_task_continuity_tools
from agent_workspace.agents.thread_state import get_thread_state_schema, normalize_middleware_state_schemas
...
from agent_workspace.config.agents_config import load_agent_config, validate_agent_name
from agent_workspace.config.app_config import AppConfig, get_app_config
from agent_workspace.config.memory_config import should_use_memory_tools
from agent_workspace.config.subagents_config import (
...
from agent_workspace.tracing import build_tracing_callbacks
...
def _subagent_release_policy(
    ...
    *,
    ...
    from agent_workspace.subagents import get_available_subagent_names, get_subagent_config
...
def _append_memory_tools_without_name_conflicts(tools: list) -> None:
    ...
    from agent_workspace.agents.memory.tools import get_memory_tools
...
def _create_todo_list_middleware(is_plan_mode: bool) -> TodoMiddleware | None:
    ...
    # Custom prompts matching Alpha's style
    system_prompt = """
<todo_list_system>
...
def build_middlewares(
    ...
    *,
    ...
    if should_use_memory_tools(resolved_app_config.memory):
        from agent_workspace.agents.memory.manager import backend_requires_passive_writes_in_tool_mode
    ...
    from agent_workspace.agents.middlewares.learning_fork_middleware import build_learning_fork_middleware
    from agent_workspace.config.learning_fork_config import get_learning_fork_config
    ...
    if deferred_setup is not None and deferred_setup.deferred_names:
        ...
        from agent_workspace.agents.middlewares.mcp_routing_middleware import assert_mcp_routing_before_deferred_filter
    ...
    from agent_workspace.extensions.policy import project_host_policy
...
def _load_enabled_available_skills(available_skills: set[str] | None, *, app_config: AppConfig, user_id: str | None = None) -> list[Skill]:
    try:
        from agent_workspace.agents.lead_agent.prompt import get_enabled_skills_for_config
...
def _complete_assembly(
    *,
    ...
    """
    from agent_workspace.diagnostics.invariants import verify_agent_assembly
    from agent_workspace.extensions import get_agent_build_extensions
...
def _assemble_lead_agent(config: RunnableConfig, *, app_config: AppConfig) -> LeadAgentAssembly:
    ...
    from agent_workspace.tools import get_available_tools
    from agent_workspace.tools.builtins import setup_agent, update_agent
    from agent_workspace.tools.builtins.tool_search import assemble_deferred_tools, build_mcp_routing_middleware, get_mcp_routing_hints_prompt_section
    ...
    from agent_workspace.runtime.user_context import resolve_config_user_id
    ...
    from agent_workspace.config.agent_preset_config import preset_summary, resolve_agent_preset
