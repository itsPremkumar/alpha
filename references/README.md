...
```mermaid
graph TD
    subgraph "Layer 3: Autonomous Scaffolding Harness (Alpha / Deep Agents)"
        ContextEng["Context Engineering & Compression"]
        VirtualFS["Virtual Workspace Filesystem"]
        PlanningEngine["Structured 5-Pass Research & Planning"]
        SubagentMgmt["Subagent Spawning & Isolation"]
        CognitiveMem["L0-L8 Multi-Tier Memory Plane"]
    end

    subgraph "Layer 2: Stateful Durable Runtime (LangGraph)"
        GraphEngine["State Graph Execution Engine"]
        Checkpointers["Temporal State Checkpointers"]
        Interrupts["Human-In-The-Loop Interrupts"]
        TimeTravel["State Time-Travel & Branching"]
    end

    subgraph "Layer 1: Primitive Framework (LangChain)"
        ModelProviders["Multi-Provider LLM Interfaces"]
        ToolDecl["Tool Declarations & Pydantic Schemas"]
        PromptTemplates["Prompt Templates & Tokenizers"]
    end

    PlanningEngine --> GraphEngine
    VirtualFS --> GraphEngine
    ContextEng --> GraphEngine
    SubagentMgmt --> GraphEngine
    GraphEngine --> ModelProviders
    GraphEngine --> ToolDecl
```
