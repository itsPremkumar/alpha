#!/usr/bin/env bash
# status.sh — Check Agent Workspace status and list available resources.
#
# Usage:
...
#   AGENT_WORKSPACE_URL           — Unified proxy base URL (default: http://localhost:2026)
#   AGENT_WORKSPACE_GATEWAY_URL   — Gateway API base URL (default: $AGENT_WORKSPACE_URL)
#   AGENT_WORKSPACE_LANGGRAPH_URL — LangGraph API base URL (default: $AGENT_WORKSPACE_URL/api/langgraph)

set -euo pipefail

AGENT_WORKSPACE_URL="${AGENT_WORKSPACE_URL:-http://localhost:2026}"
GATEWAY_URL="${AGENT_WORKSPACE_GATEWAY_URL:-$AGENT_WORKSPACE_URL}"
LANGGRAPH_URL="${AGENT_WORKSPACE_LANGGRAPH_URL:-$AGENT_WORKSPACE_URL/api/langgraph}"
CMD="${1:-health}"
ARG="${2:-}"

case "$CMD" in
  health)
    echo "Checking Agent Workspace at ${GATEWAY_URL}..."
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${GATEWAY_URL}/health" 2>/dev/null || echo "000")
    if [ "$HTTP_CODE" = "000" ]; then
      echo "UNREACHABLE — Agent Workspace is not running at ${GATEWAY_URL}"
      exit 1
    elif [ "$HTTP_CODE" -ge 400 ]; then
      echo "ERROR — Health check returned HTTP ${HTTP_CODE}"
      exit 1
    else
      echo "OK — Agent Workspace is running (HTTP ${HTTP_CODE})"
    fi
    ;;
  models)
