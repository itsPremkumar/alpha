...
SKIP_DIRS = {".git", "node_modules", ".venv", "dist", ".next", "build", "__pycache__", "coverage", ".pytest_cache", ".mypy_cache", "sandbox", "logs", "site"}
SOURCE_SUFFIXES = {".py", ".ps1", ".sh", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".yaml", ".yml", ".toml", ".json", ".md", ".example", ".env", ".cfg", ".ini"}
...
PATTERNS: list[tuple[re.Pattern[str], int, str, frozenset[str] | None]] = [
    ...
    (re.compile(r"\$(?:\{|\()?([A-Z][A-Z0-9_]{2,})"), 1, "read", frozenset({".sh", ".ps1", ""})),  # shell $X / ${X} / $(VAR) incl. Makefile
...
def iter_files():
    ...
    for dirpath, dirnames, filenames in os.walk(ROOT):
        ...
        for name in filenames:
            p = Path(dirpath) / name
            if (
                p.suffix.lower() not in SOURCE_SUFFIXES
                and name
                not in {
                    "Dockerfile",
                    ".env",
...
def main() -> int:
    ...
    fam_names = sorted(n for n in set(set_by) | set(read_by) if n.startswith(FAMILIES))
    dead_set: list[str] = []
    set_only: list[str] = []
    read_only: list[str] = []
    for n in fam_names:
    ...
    print(f"rename-family variables: {len(fam_names)} (set={len(set_by)}, read={len(read_by)})")
    print("-" * 72)
    for n in dead_set:
