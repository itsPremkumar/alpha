...
if [ -z "$AGENT_WORKSPACE_HOME" ]; then
    export AGENT_WORKSPACE_HOME="$REPO_ROOT/backend/.agent-workspace"
fi
echo -e "${BLUE}AGENT_WORKSPACE_HOME=$AGENT_WORKSPACE_HOME${NC}"
mkdir -p "$AGENT_WORKSPACE_HOME"

# ── AGENT_WORKSPACE_REPO_ROOT (for skills host path in DooD) ───────────────────────

export AGENT_WORKSPACE_REPO_ROOT="$REPO_ROOT"

# ── config.yaml ───────────────────────────────────────────────────────────────

if [ -z "$AGENT_WORKSPACE_CONFIG_PATH" ]; then
    export AGENT_WORKSPACE_CONFIG_PATH="$REPO_ROOT/config.yaml"
fi

if  [ "$CMD" != "down" ] && [ ! -f "$AGENT_WORKSPACE_CONFIG_PATH" ]; then
    # Try to seed from repo (config.example.yaml is the canonical template)
    if [ -f "$REPO_ROOT/config.example.yaml" ]; then
        cp "$REPO_ROOT/config.example.yaml" "$AGENT_WORKSPACE_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded config.example.yaml → $AGENT_WORKSPACE_CONFIG_PATH${NC}"
        echo -e "${YELLOW}⚠ config.yaml was seeded from the example template.${NC}"
        echo "  Run 'make setup' to generate a minimal config, or edit $AGENT_WORKSPACE_CONFIG_PATH manually before use."
    else
    ...
    echo -e "${GREEN}✓ config.yaml: $AGENT_WORKSPACE_CONFIG_PATH${NC}"
fi

# ── extensions_config.json ───────────────────────────────────────────────────

if [ -z "$AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH" ]; then
    export AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH="$REPO_ROOT/extensions_config.json"
fi

if [ ! -f "$AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH" ]; then
    if [ -f "$REPO_ROOT/extensions_config.json" ]; then
        cp "$REPO_ROOT/extensions_config.json" "$AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded extensions_config.json → $AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH${NC}"
    else
        # Create a minimal empty config so the gateway doesn't fail on startup
        echo '{"mcpServers":{},"skills":{}}' > "$AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH"
        echo -e "${YELLOW}⚠ extensions_config.json not found, created empty config at $AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH${NC}"
    fi
else
    echo -e "${GREEN}✓ extensions_config.json: $AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH${NC}"
fi
...
_secret_file="$AGENT_WORKSPACE_HOME/.better-auth-secret"
...
# ── AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN ────────────────────────────────────────────
...
_internal_auth_token_file="$AGENT_WORKSPACE_HOME/.internal-auth-token"
if  [ "$CMD" != "down" ] && [ -z "$AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN" ]; then
    if [ -f "$_internal_auth_token_file" ]; then
        export AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN
        AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN="$(cat "$_internal_auth_token_file")"
        echo -e "${GREEN}✓ AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN loaded from $_internal_auth_token_file${NC}"
    else
        export AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN
        if command -v python3 > /dev/null 2>&1 && \
            AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN="$(python3 -c 'import sys; sys.version_info >= (3, 6) or sys.exit(1); import secrets; print(secrets.token_urlsafe(32))' 2>/dev/null)"; then
            true
        elif command -v python > /dev/null 2>&1 && \
            AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN="$(python -c 'import sys; sys.version_info >= (3, 6) or sys.exit(1); import secrets; print(secrets.token_urlsafe(32))' 2>/dev/null)"; then
            true
        elif command -v openssl > /dev/null 2>&1 && \
            AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN="$(openssl rand -hex 32)"; then
            true
        else
            echo -e "${RED}✗ Cannot generate AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN: python3, python, and openssl are all unavailable.${NC}" >&2
            echo -e "${RED}  Set AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN manually before running make up.${NC}" >&2
            exit 1
        fi
        echo "$AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN" > "$_internal_auth_token_file"
        chmod 600 "$_internal_auth_token_file"
        echo -e "${GREEN}✓ AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN generated → $_internal_auth_token_file${NC}"
    fi
fi
...
detect_sandbox_mode() {
    ...
    [ -f "$AGENT_WORKSPACE_CONFIG_PATH" ] || { echo "local"; return; }
...
if [ "$CMD" = "down" ]; then
    ...
    export AGENT_WORKSPACE_HOME="${AGENT_WORKSPACE_HOME:-$REPO_ROOT/backend/.agent-workspace}"
    export AGENT_WORKSPACE_CONFIG_PATH="${AGENT_WORKSPACE_CONFIG_PATH:-$AGENT_WORKSPACE_HOME/config.yaml}"
    export AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH="${AGENT_WORKSPACE_EXTENSIONS_CONFIG_PATH:-$AGENT_WORKSPACE_HOME/extensions_config.json}"
    export AGENT_WORKSPACE_REPO_ROOT="${AGENT_WORKSPACE_REPO_ROOT:-$REPO_ROOT}"
    export BETTER_AUTH_SECRET="${BETTER_AUTH_SECRET:-placeholder}"
    export AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN="${AGENT_WORKSPACE_INTERNAL_AUTH_TOKEN:-placeholder}"
    "${COMPOSE_CMD[@]}" down
    exit 0
fi
...
if [ "$CMD" = "build" ]; then
    ...
    echo "=========================================="
    echo ""
    echo "Run 'deploy.sh start' or 'make deploy-start' to start services."
    exit 0
fi
...
# ── AGENT_WORKSPACE_DOCKER_SOCKET (aio / pure-DooD mode only) ──────────────────────
...
if [ -z "$AGENT_WORKSPACE_DOCKER_SOCKET" ]; then
    export AGENT_WORKSPACE_DOCKER_SOCKET="/var/run/docker.sock"
fi

if [ "$sandbox_mode" = "aio" ]; then
    if [ ! -S "$AGENT_WORKSPACE_DOCKER_SOCKET" ]; then
        ...
        if [ "$AGENT_WORKSPACE_DOCKER_SOCKET" = "/var/run/docker.sock" ] && [[ "$(uname -s)" =~ ^(MINGW|MSYS|CYGWIN) ]] && docker info >/dev/null 2>&1; then
            :
        else
            echo -e "${RED}⚠ Docker socket not found at $AGENT_WORKSPACE_DOCKER_SOCKET${NC}"
            echo "  AioSandboxProvider (DooD) will not work."
            exit 1
        fi
    fi
    echo -e "${GREEN}✓ Docker socket: $AGENT_WORKSPACE_DOCKER_SOCKET${NC}"
