...
CHART="$ROOT/deploy/helm/agent-workspace"
...
if ! helm template agent-workspace "$CHART" --include-crds >"$TMP/default.yaml"; then
  echo "::error::default chart render failed" >&2
  exit 1
fi
if ! helm template agent-workspace "$CHART" --include-crds \
