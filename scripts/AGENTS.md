...
boundaries under `backend/app/` and `backend/packages/harness/agent_workspace/`. It
...
The `detect-blocking-io` target parses `app/`, `packages/harness/agent_workspace/`,
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/agent_workspace/` never imports from `app.*`
