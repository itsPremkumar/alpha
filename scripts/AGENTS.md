...
boundaries under `backend/app/` and `backend/packages/harness/alpha/`. It
prints a concise count by execution domain and writes the complete, versioned
JSON payload to `.agent-workspace/thread-boundary-inventory.json`. Every finding has
...
python scripts/detect_thread_boundaries.py \
  ...
  --json-output .agent-workspace/thread-boundary-inventory.json
```
...
The `detect-blocking-io` target parses `app/`, `packages/harness/alpha/`,
...
findings to `.agent-workspace/blocking-io-findings.json` at the repository root
...
- Wraps every item under `tests/blocking_io/` with a strict Blockbuster
  context scoped to `app.*` and `alpha.*` (see
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/alpha/` never imports from `app.*`
