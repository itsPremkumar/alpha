...
def _candidate_thread_data_dirs(project_root: Path, thread_id: str) -> list[Path]:
    ...
    candidates = [
        ...
        project_root / "backend" / ".agent-workspace" / "threads" / thread_id / "user-data",
    ]
    for base in (project_root / ".agent-workspace" / "users", project_root / "backend" / ".agent-workspace" / "users"):
        if base.exists():
