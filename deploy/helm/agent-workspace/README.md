...
```bash
helm install agent-workspace oci://ghcr.io/<owner>/charts/agent-workspace \
  --version <version> \
...
config: |
  ...
  stream_bridge:
    type: redis   # cross-pod SSE; URL from AGENT_WORKSPACE_STREAM_BRIDGE_REDIS_URL
...
`/app/backend/.agent-workspace/extensions-config/extensions_config.json`, where the
...
- **Redis stream bridge.** A bundled single-instance redis StatefulSet
  ...
  `AGENT_WORKSPACE_STREAM_BRIDGE_REDIS_URL`; `config.yaml` sets `stream_bridge.type:
...
|---|---|---|---|
| gateway | 1000 | 1000 | `.agent-workspace` PVC group-writable via fsGroup; `PYTHONDONTWRITEBYTECODE=1` suppresses `.pyc` writes; `UV_CACHE_DIR=/tmp` |
